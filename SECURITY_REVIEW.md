...
This review covers the React/Vite implementation, Studio route, secure static server, prerendered public routes, server CMS persistence, bearer-token Studio auth, optional MFA, upload processing, local storage fallback, tracking pages, analytics, and contact/newsletter form endpoints.

## Implemented Controls

- `/admin` no longer exposes editing controls.
- Studio lives at `/studio`; `/admin` stays inert for accidental discovery and legacy probes.
- Secure mode verifies the Studio passphrase server-side and returns a short-lived bearer token for CMS, tracking, analytics and upload requests.
...
- `npm run serve:secure` protects Studio data mutations and uploads through bearer-token API checks.
- The secure server adds rate limiting, audit logging, server-side lead validation, local lead storage, optional signed contact/newsletter webhook forwarding, Royal Mail tracking lookup hooks, and `/api/ops/health`.
- Studio/editor code is split into a dedicated `studio-*` asset chunk that is not referenced by public prerendered HTML.
...
- A public Studio bundle is not a security boundary by itself; protected data changes must stay behind the secure API checks.
...
- Bearer tokens are stored by the browser session; users need to sign in again after clearing session storage or token expiry.
- TOTP MFA, CMS encryption, tracking lookup and webhook HMAC verification only apply when the corresponding environment variables are configured.
...
- Keep security headers, CSP, rate limiting, bearer-token authorization, and server-side input validation enabled in the deployment target.
