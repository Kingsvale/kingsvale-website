...
export const WORD_QR_EXPORT_SIZE = 1155;

export function buildStyledQrSvg(value, style = {}, title = "", options = {}) {
  ...
  const brand = escapeText(String(title).slice(0, 28));
...
function moduleShape(x, y, size, radius, color) {
  const left = x * size;
  const top = y * size;
...
function frameShape(x, y, width, height, roundness, cutPercent, fill, stroke, strokeWidth = 0) {
...
function logoMark(size, moduleSize, background, foreground, accent) {
